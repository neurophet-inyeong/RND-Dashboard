    ...
    const state = {
      ...
      activeProject: "",
      activeSheetTab: "사업비",
      view: "budgetLedger",   // "project" | "budgetLedger"
      ledger: null,
    ...
    // 총괄표의 과제번호가 projects/ 폴더의 엑셀 파일명(state.projects[].name)과 일치하면 해당 프로젝트로 이동
    function findMatchingLedgerProject(taskNumber) {
      const num = String(taskNumber ?? "").trim();
      if (!num) return null;
      const match = state.projects.find((p) => p.name === num);
    ...
    function renderLedgerTableView(body, summary, detail) {
      ...
      const tbody = `<tbody>${sorted
        .map((row) => {
          const cells = LEDGER_TABLE_COLUMNS.map((c) => {
            ...
            if (c.key === "과제번호") {
              const matchName = findMatchingLedgerProject(raw);
              if (matchName) {
                return `<td class="${c.cls || ""}"><a href="#" class="ledger-open-project" data-project="${matchName}" style="color:#137a2c;font-weight:700;text-decoration:none;" title="과제 시트로 이동">${text} 📄</a></td>`;
              }
      ...
      body.innerHTML = `<h3 class="card-title">과제별 상세 (총괄표 + 연차별 예산 breakdown)</h3>
        <p class="hint">과제명을 클릭하면 상세로 이동합니다. 과제번호(📄)가 있으면 클릭 시 과제 시트로 이동합니다.</p>
      ...
      const openProjectSheet = (projectName) => {
      ...
      body.querySelectorAll("tr.ov-row").forEach((tr) => {
        ...
        tr.addEventListener("click", (e) => {
          if (e.target.closest("a.ledger-open-detail, a.ledger-open-project")) return; // 링크 클릭은 자체 핸들러가 처리
      ...
      body.querySelectorAll("a.ledger-open-project").forEach((a) => {
        a.addEventListener("click", (e) => {
          e.preventDefault();
          e.stopPropagation();
          openProjectSheet(a.dataset.project);
        });
      });
    }
      ...
      budgetLedgerGrid.innerHTML = `
        ...
        <div class="card" style="margin-bottom:16px;">
          ...
          <div style="height:660px;">
            <canvas id="ledgerYearlyCostChart"></canvas>
          </div>
        </div>
      `;
    ...
    async function parseFiles(fileEntries) {
      ...
      for (const fileEntry of fileEntries) {
        ...
        wb.SheetNames.forEach((sheetName) => {
          ...
          } else if (sheetName === "개요") {
            // 개요 시트는 상단 개요 블록(A2:B15) + 하단 사업비 표가 한 시트에 합쳐져 있음
            const combined = parseCombinedSheet(wb, sheetName);
            if (combined) {
              sheets["개요"] = combined.overviewRows;
              if (!sheets["사업비"]) sheets["사업비"] = combined.budgetRows;
            } else {
              sheets[sheetName] = sheetRows(wb, sheetName, 1);
            }
          } else {
            // 정량지표상세 등 다른 시트는 단일 헤더 표이므로 결합 시트 파싱을 적용하지 않음
            // (parseCombinedSheet는 개요 블록 감지를 위해 A/B열을 무조건 훑기 때문에
            //  다른 시트에 적용하면 오탐으로 실제 데이터가 빈 배열로 덮어써짐)
            sheets[sheetName] = sheetRows(wb, sheetName, 1);
          }
        });
