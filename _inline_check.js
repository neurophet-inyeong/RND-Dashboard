    ...
    function groupSumBy(rows, groupKey, valueKey) {
      const map = new Map();
      for (const r of rows) {
        const g = String(r[groupKey] ?? "").trim() || "미지정";
        map.set(g, (map.get(g) || 0) + (parseNumericLike(r[valueKey]) || 0));
      }
    ...
    // 원본데이터의 시작일 연도 기준으로 비용 항목(인건비/연구활동비 등)을 합산
    ...
    function renderLedgerTableView(body, summary, detail) {
      ...
      const thead = `<thead><tr>${LEDGER_TABLE_COLUMNS.map((c) => `<th>${c.label}</th>`).join("")}</tr></thead>`;
      const tbody = `<tbody>${sorted
        .map((row) => {
          const cells = LEDGER_TABLE_COLUMNS.map((c) => {
            ...
            if (c.key === "과제명") {
              return `<td><a href="#" class="ledger-open-detail" data-num="${row["번호"]}" style="color:var(--accent);font-weight:700;text-decoration:none;">${text || "(과제명 미입력)"}</a></td>`;
            }
            return `<td>${text}</td>`;
          }).join("");
      ...
      body.innerHTML = `<h3 class="card-title">과제별 상세 (총괄표 + 연차별 예산 breakdown)</h3>
        ...
        <div class="table-wrap" style="max-height:520px;">
          <table class="sheet-table ov-table">${thead}${tbody}</table>
        </div>`;
    ...
    async function renderBudgetLedger() {
      ...
      const typeData = groupSumBy(summary, "유형", "총 사업비");
      ...
      budgetLedgerGrid.innerHTML = `
        ...
        <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:16px;margin-bottom:16px;">
          <div class="card"><h3 class="card-title">연도별 비용 항목 합계 (원본데이터)</h3><canvas id="ledgerYearlyCostChart" height="220"></canvas></div>
          <div class="card"><h3 class="card-title">R&amp;D / 비R&amp;D 비중</h3><canvas id="ledgerTypeChart" height="220"></canvas></div>
        </div>
      `;
      ...
      ledgerYearlyCostChart = new Chart(yearlyCostCtx, {
        ...
        data: {
          ...
          datasets: yearlyCost.categories.map((cat, i) => ({
            label: cat.label,
        ...
        options: {
          ...
          scales: {
            x: { stacked: true },
            y: { stacked: true, ticks: { callback: (v) => Number(v).toLocaleString("ko-KR") } }
          }
        }
      });

      if (ledgerTypeChart) { ledgerTypeChart.destroy(); ledgerTypeChart = null; }
      const typeCtx = document.getElementById("ledgerTypeChart");
      ledgerTypeChart = new Chart(typeCtx, {
        type: "pie",
        data: {
          labels: typeData.map(([k]) => k),
          datasets: [{ data: typeData.map(([, v]) => v), backgroundColor: ["#4a66ff", "#27ae6f", "#f1a94d", "#e0763f"] }]
        }
      });
